...
public:
	...
	UFUNCTION()
	MODULE_API void StartHoldProgress(FName HoldActionName, float HoldDuration);
...
protected:
	MODULE_API virtual void NativePreConstruct() override;
	MODULE_API virtual void NativeConstruct() override;
	MODULE_API virtual int32 NativePaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const override;
	MODULE_API void RecalculateDesiredSize();
...
private:
	 ...
	 */
	MODULE_API void SyncHoldProgress();

	/** Called for updating the HoldKeybindImage during a hold keybind */
	MODULE_API void UpdateHoldProgress();

	/** Called when we want to set up this keybind widget as a hold keybind */
	MODULE_API void SetupHoldKeybind();

	MODULE_API void ShowHoldBackPlate();

	MODULE_API void HandlePlayerControllerSet(UCommonLocalPlayer* LocalPlayer, APlayerController* PlayerController);
