...
#define MODULE_API GAMEPLAYMESSAGERUNTIME_API
...
public:
	 ...
	 */
	static MODULE_API UGameplayMessageSubsystem& Get(const UObject* WorldContextObject);

	/**
	 * @return true if a valid GameplayMessageRouter subsystem if active in the provided world
	 */
	static MODULE_API bool HasInstance(const UObject* WorldContextObject);
	 ...
	 */
	MODULE_API void UnregisterListener(FGameplayMessageListenerHandle Handle);
...
protected:
	 ...
	 * @param Message			The message to send (must be the same type of UScriptStruct expected by the listeners for this channel, otherwise an error will be logged)
	 */
	UFUNCTION(Category = "Messaging", BlueprintCallable, CustomThunk, meta = (CustomStructureParam="Message", AllowAbstract="false", DisplayName="Broadcast Message"))
	MODULE_API void K2_BroadcastMessage(FGameplayTag Channel, const int32& Message);

	DECLARE_FUNCTION(execK2_BroadcastMessage);

private:
	// Internal helper for broadcasting a message
	MODULE_API void BroadcastMessageInternal(FGameplayTag Channel, const UScriptStruct* StructType, const void* MessageBytes);

	// Internal helper for registering a message listener
	MODULE_API FGameplayMessageListenerHandle RegisterListenerInternal(
		FGameplayTag Channel, 
		TFunction<void(FGameplayTag, const UScriptStruct*, const void*)>&& Callback,
		const UScriptStruct* StructType,
		EGameplayMessageMatch MatchType);

	MODULE_API void UnregisterListenerInternal(FGameplayTag Channel, int32 HandleID);
