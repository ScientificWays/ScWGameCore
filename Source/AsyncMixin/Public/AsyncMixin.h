...
#define MODULE_API ASYNCMIXIN_API
...
{
protected:
	MODULE_API FAsyncMixin();

public:
	MODULE_API virtual ~FAsyncMixin();
...
protected:
	...
	MODULE_API void AsyncLoad(FSoftObjectPath SoftObjectPath, const FSimpleDelegate& Callback = FSimpleDelegate());
	...
	/** Async load an array of FSoftObjectPath, call the Callback when complete. */
	MODULE_API void AsyncLoad(const TArray<FSoftObjectPath>& SoftObjectPaths, const FSimpleDelegate& Callback = FSimpleDelegate());
	...
	/** Given an array of primary asset ids, it loads all of the bundles referenced by properties of these assets specified in the LoadBundles array. */
	MODULE_API void AsyncPreloadPrimaryAssetsAndBundles(const TArray<FPrimaryAssetId>& AssetIds, const TArray<FName>& LoadBundles, const FSimpleDelegate& Callback = FSimpleDelegate());

	/** Add a future condition that must be true before we move forward. */
	MODULE_API void AsyncCondition(TSharedRef<FAsyncCondition> Condition, const FSimpleDelegate& Callback = FSimpleDelegate());
	 ...
	 * tied to a particular asset in case some of the assets are optional.
	 */
	MODULE_API void AsyncEvent(const FSimpleDelegate& Callback);

	/** Flushes any async loading requests. */
	MODULE_API void StartAsyncLoading();

	/** Cancels any pending async loads. */
	MODULE_API void CancelAsyncLoading();

	/** Is async loading current in progress? */
	MODULE_API bool IsAsyncLoadingInProgress() const;
...
private:
	...
	MODULE_API const FLoadingState& GetLoadingStateConst() const;
	
	MODULE_API FLoadingState& GetLoadingState();

	MODULE_API bool HasLoadingState() const;

	MODULE_API bool IsLoadingInProgressOrPending() const;

private:
	static MODULE_API TMap<FAsyncMixin*, TSharedRef<FLoadingState>> Loading;
};
