...
public:
	...
	MODULE_API virtual void Tick(float DeltaTime) override;
	MODULE_API virtual ETickableTickType GetTickableTickType() const override;
	MODULE_API virtual bool IsTickable() const override;
	MODULE_API virtual TStatId GetStatId() const override;
	...
	MODULE_API void RegisterLoadingProcessor(TScriptInterface<ILoadingProcessInterface> Interface);
	MODULE_API void UnregisterLoadingProcessor(TScriptInterface<ILoadingProcessInterface> Interface);
	
private:
	MODULE_API void HandlePreLoadMap(const FWorldContext& WorldContext, const FString& MapName);
	MODULE_API void HandlePostLoadMap(UWorld* World);

	/** Determines if we should show or hide the loading screen. Called every frame. */
	MODULE_API void UpdateLoadingScreen();

	/** Returns true if we need to be showing the loading screen. */
	MODULE_API bool CheckForAnyNeedToShowLoadingScreen();
