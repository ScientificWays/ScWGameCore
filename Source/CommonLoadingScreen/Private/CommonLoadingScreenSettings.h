...
public:
	...
	UPROPERTY(Category = "Display", config, EditAnywhere, meta = (MetaClass="/Script/UMG.UserWidget"))
	FSoftClassPath LoadingScreenWidget;

	// The z-order of the loading screen widget in the viewport stack
	UPROPERTY(Category = "Display", config, EditAnywhere)
	...
	// enabled via HoldLoadingScreenAdditionalSecsEvenInEditor
 	UPROPERTY(Category = "Configuration", config, EditAnywhere, meta = (ForceUnits=s, ConsoleVariable="CommonLoadingScreen.HoldLoadingScreenAdditionalSecs"))
	...
	// The interval in seconds beyond which the loading screen is considered permanently hung (if non-zero).
 	UPROPERTY(Category = "Configuration", config, EditAnywhere, meta = (ForceUnits=s))
	...
	UPROPERTY(Category = "Debugging", Transient, EditAnywhere, meta = (ConsoleVariable="CommonLoadingScreen.LogLoadingScreenReasonEveryFrame"))
	bool LogLoadingScreenReasonEveryFrame = 0;

	// Force the loading screen to be displayed (useful for debugging)
	UPROPERTY(Category = "Debugging", Transient, EditAnywhere, meta = (ConsoleVariable="CommonLoadingScreen.AlwaysShow"))
	...
	UPROPERTY(Category = "Debugging", Transient, EditAnywhere)
	bool HoldLoadingScreenAdditionalSecsEvenInEditor = false;

	// Should we apply the additional HoldLoadingScreenAdditionalSecs delay even in the editor
	// (useful when iterating on loading screens)
	UPROPERTY(Category = "Configuration", config, EditAnywhere)
