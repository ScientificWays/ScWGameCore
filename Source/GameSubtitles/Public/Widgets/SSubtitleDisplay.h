...
public:
	...
	MODULE_API ~SSubtitleDisplay();

	MODULE_API void Construct( const FArguments& InArgs );

	MODULE_API void SetTextStyle(const FTextBlockStyle& InTextStyle);

	MODULE_API void SetBackgroundBrush(const FSlateBrush* InSlateBrush);

	MODULE_API void SetCurrentSubtitleText(const FText& SubtitleText);

	MODULE_API bool HasSubtitles() const;

	/** See WrapTextAt attribute */
	MODULE_API void SetWrapTextAt(const TAttribute<float>& InWrapTextAt);
