...
#include "ScWAudioMixEffectsSubsystem.generated.h"
...
USTRUCT()
struct FScWAudioSubmixEffectsChain
{
	GENERATED_BODY()
 ...
 */
UCLASS(MinimalAPI)
class UScWAudioMixEffectsSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
public:
	...
	MODULE_API virtual void PostInitialize() override;

	/** Called when world is ready to start gameplay before the game mode transitions to the correct state and call BeginPlay on all actors */
	MODULE_API virtual void OnWorldBeginPlay(UWorld& InWorld) override;

	/** Set whether the HDR Audio Submix Effect Chain Override settings are applied */
	MODULE_API void ApplyDynamicRangeEffectsChains(bool bHDRAudio);
	
protected:
	MODULE_API void OnLoadingScreenStatusChanged(bool bShowingLoadingScreen);
	MODULE_API void ApplyOrRemoveLoadingScreenMix(bool bWantsLoadingScreenMix);
	
	// Called when determining whether to create this Subsystem
	MODULE_API virtual bool DoesSupportWorldType(const EWorldType::Type WorldType) const override;

	// Default Sound Control Bus Mix retrieved from the ScW Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> DefaultBaseMix = nullptr;

	// Loading Screen Sound Control Bus Mix retrieved from the ScW Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> LoadingScreenMix = nullptr;

	// User Sound Control Bus Mix retrieved from the ScW Audio Settings
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBusMix> UserMix = nullptr;

	// Overall Sound Control Bus retrieved from the ScW Audio Settings and linked to the UI and game settings in ScWSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> OverallControlBus = nullptr;

	// Music Sound Control Bus retrieved from the ScW Audio Settings and linked to the UI and game settings in ScWSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> MusicControlBus = nullptr;

	// SoundFX Sound Control Bus retrieved from the ScW Audio Settings and linked to the UI and game settings in ScWSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> SoundFXControlBus = nullptr;

	// Dialogue Sound Control Bus retrieved from the ScW Audio Settings and linked to the UI and game settings in ScWSettingsLocal
	UPROPERTY(Transient)
	TObjectPtr<USoundControlBus> DialogueControlBus = nullptr;

	// VoiceChat Sound Control Bus retrieved from the ScW Audio Settings and linked to the UI and game settings in ScWSettingsLocal
	UPROPERTY(Transient)
	...
	UPROPERTY(Transient)
	TArray<FScWAudioSubmixEffectsChain> HDRSubmixEffectChain;

	// Submix Effect hain Overrides to apply when HDR Audio is turned off
	UPROPERTY(Transient)
	TArray<FScWAudioSubmixEffectsChain> LDRSubmixEffectChain;
