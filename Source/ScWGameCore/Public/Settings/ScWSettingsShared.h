...
#include "ScWSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EScWAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
public:
	DECLARE_EVENT_OneParam(UScWSettingsShared, FOnSettingChangedEvent, UScWSettingsShared* Settings);
...
public:
	...
	static MODULE_API UScWSettingsShared* CreateTemporarySettings(const UScWLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static MODULE_API UScWSettingsShared* LoadOrCreateSettings(const UScWLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UScWSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static MODULE_API bool AsyncLoadOrCreateSettings(const UScWLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	EScWAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EScWAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EScWAllowBackgroundAudioSetting AllowAudioInBackground = EScWAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EScWGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EScWGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EScWGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EScWGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EScWGamepadSensitivity GamepadLookSensitivityPreset = EScWGamepadSensitivity::Normal;
	UPROPERTY()
	EScWGamepadSensitivity GamepadTargetingSensitivityPreset = EScWGamepadSensitivity::Normal;
