...
class UScWPerformanceStatSubsystem;
class UObject;
struct FFrame;
...
struct FScWPerformanceStatCache : public IPerformanceDataConsumer
{
public:
	FScWPerformanceStatCache(UScWPerformanceStatSubsystem* InSubsystem)
...
protected:
	...
	void RecordStat(const EScWDisplayablePerformanceStat Stat, const double Value);
	
	UScWPerformanceStatSubsystem* MySubsystem;

	/**
	 * Caches the sampled data for each of the performance stats currently available
	 */
	TMap<EScWDisplayablePerformanceStat, FSampledStatCache> PerfStateCache;
};
