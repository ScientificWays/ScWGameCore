...
#include "Features/ScWGFA_AddAbilities.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ScWGFA_AddAbilities)

#define LOCTEXT_NAMESPACE "GameFeatures"

//////////////////////////////////////////////////////////////////////
// UScWGFA_AddAbilities

void UScWGFA_AddAbilities::OnGameFeatureActivating(FGameFeatureActivatingContext& Context)
{
...
void UScWGFA_AddAbilities::OnGameFeatureDeactivating(FGameFeatureDeactivatingContext& Context)
{
...
#if WITH_EDITOR
EDataValidationResult UScWGFA_AddAbilities::IsDataValid(FDataValidationContext& Context) const
{
	...
	for (const FGameFeatureAbilitiesEntry& Entry : AbilitiesList)
	{
		...
		for (const FScWAbilityGrant& Ability : Entry.GrantedAbilities)
		{
		...
		for (const FScWAttributeSetGrant& Attributes : Entry.GrantedAttributes)
		{
		...
		for (const TSoftObjectPtr<const UScWAbilitySet>& AttributeSetPtr : Entry.GrantedAbilitySets)
		{
...
void UScWGFA_AddAbilities::AddToWorld(const FWorldContext& WorldContext, const FGameFeatureStateChangeContext& ChangeContext)
{
			...
			for (const FGameFeatureAbilitiesEntry& Entry : AbilitiesList)
			{
				if (!Entry.ActorClass.IsNull())
				{
					UGameFrameworkComponentManager::FExtensionHandlerDelegate AddAbilitiesDelegate = UGameFrameworkComponentManager::FExtensionHandlerDelegate::CreateUObject(
						this, &UScWGFA_AddAbilities::HandleActorExtension, EntryIndex, ChangeContext);
...
void UScWGFA_AddAbilities::Reset(FPerContextData& ActiveData)
{
...
void UScWGFA_AddAbilities::HandleActorExtension(AActor* Actor, FName EventName, int32 EntryIndex, FGameFeatureStateChangeContext ChangeContext)
{
...
void UScWGFA_AddAbilities::AddActorAbilities(AActor* Actor, const FGameFeatureAbilitiesEntry& AbilitiesEntry, FPerContextData& ActiveData)
{
	check(Actor);
	...
	if (UAbilitySystemComponent* AbilitySystemComponent = FindOrAddComponentForActor<UAbilitySystemComponent>(Actor, AbilitiesEntry, ActiveData))
	{
		...
		for (const FScWAbilityGrant& Ability : AbilitiesEntry.GrantedAbilities)
		{
			if (!Ability.AbilityType.IsNull())
			{
		...
		for (const FScWAttributeSetGrant& Attributes : AbilitiesEntry.GrantedAttributes)
		{
			if (!Attributes.AttributeSetType.IsNull())
			{
		...
		UScWAbilitySystemComponent* ScWASC = CastChecked<UScWAbilitySystemComponent>(AbilitySystemComponent);
		for (const TSoftObjectPtr<const UScWAbilitySet>& SetPtr : AbilitiesEntry.GrantedAbilitySets)
		{
			if (const UScWAbilitySet* Set = SetPtr.Get())
			{
				Set->GiveToAbilitySystem(ScWASC, &AddedExtensions.AbilitySetHandles.AddDefaulted_GetRef());
			}
		}
...
void UScWGFA_AddAbilities::RemoveActorAbilities(AActor* Actor, FPerContextData& ActiveData)
{
	if (FActorExtensions* ActorExtensions = ActiveData.ActiveExtensions.Find(Actor))
	{
		if (UAbilitySystemComponent* AbilitySystemComponent = Actor->FindComponentByClass<UAbilitySystemComponent>())
		{
			...
			UScWAbilitySystemComponent* ScWASC = CastChecked<UScWAbilitySystemComponent>(AbilitySystemComponent);
			for (FScWAbilitySet_GrantedHandles& SetHandle : ActorExtensions->AbilitySetHandles)
			{
				SetHandle.TakeFromAbilitySystem(ScWASC);
			}
		}
...
UActorComponent* UScWGFA_AddAbilities::FindOrAddComponentForActor(UClass* ComponentType, AActor* Actor, const FGameFeatureAbilitiesEntry& AbilitiesEntry, FPerContextData& ActiveData)
{
