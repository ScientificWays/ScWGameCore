...
#include "Input/ScWPlayerInput.h"

#include "Settings/ScWSettingsLocal.h"

#include "Performance/LatencyMarkerModule.h"

UScWPlayerInput::UScWPlayerInput()
	: Super()
{
...
bool UScWPlayerInput::InputKey(const FInputKeyEventArgs& Params)
{
	const bool bResult = Super::InputKey(Params);

	// Note: Since ScW is only going to support the "Reflex" plugin to handle latency markers,
...
void UScWPlayerInput::ProcessInputEventForLatencyMarker(const FInputKeyEventArgs& Params)
{
...
void UScWPlayerInput::BindToLatencyMarkerSettingChange()
{
...
void UScWPlayerInput::UnbindLatencyMarkerSettingChangeListener()
{
	UScWSettingsLocal* Settings = UScWSettingsLocal::Get();
	if (!Settings)
	{
...
void UScWPlayerInput::HandleLatencyMarkerSettingChanged()
{
	// Make sure that we only ever get this callback on platforms which support latency markers
	ensure(UScWSettingsLocal::DoesPlatformSupportLatencyMarkers());
