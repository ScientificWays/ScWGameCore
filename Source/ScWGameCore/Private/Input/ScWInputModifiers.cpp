...
#include "Input/ScWInputModifiers.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ScWInputModifiers)

DEFINE_LOG_CATEGORY_STATIC(LogScWInputModifiers, Log, All);

//////////////////////////////////////////////////////////////////////
// FScWInputModifiersHelpers

namespace FScWInputModifiersHelpers
{
	/** Returns the owning ScWLocalPlayer of an Enhanced Player Input pointer */
	static UScWLocalPlayer* GetLocalPlayer(const UEnhancedPlayerInput* PlayerInput)
	{
		if (PlayerInput)
		{
			if (APlayerController* PC = Cast<APlayerController>(PlayerInput->GetOuter()))
			{
				return Cast<UScWLocalPlayer>(PC->GetLocalPlayer());
			}
		}
		return nullptr;
	}
...
//////////////////////////////////////////////////////////////////////
// UScWSettingBasedScalar

FInputActionValue UScWSettingBasedScalar::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	...
	{
		if (UScWLocalPlayer* LocalPlayer = FScWInputModifiersHelpers::GetLocalPlayer(PlayerInput))
		{
			const UClass* SettingsClass = UScWSettingsShared::StaticClass();
			UScWSettingsShared* SharedSettings = LocalPlayer->GetSharedSettings();
...
//////////////////////////////////////////////////////////////////////
// UScWInputModifierDeadZone

FInputActionValue UScWInputModifierDeadZone::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	EInputActionValueType ValueType = CurrentValue.GetValueType();
	UScWLocalPlayer* LocalPlayer = FScWInputModifiersHelpers::GetLocalPlayer(PlayerInput);
	if (ValueType == EInputActionValueType::Boolean || !LocalPlayer)
	{
		return CurrentValue;
	}
	
	UScWSettingsShared* Settings = LocalPlayer->GetSharedSettings();
	ensure(Settings);
...
FLinearColor UScWInputModifierDeadZone::GetVisualizationColor_Implementation(FInputActionValue SampleValue, FInputActionValue FinalValue) const
{
...
//////////////////////////////////////////////////////////////////////
// UScWInputModifierGamepadSensitivity

FInputActionValue UScWInputModifierGamepadSensitivity::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	...
	const EScWGamepadSensitivity Sensitivity = (TargetingType == EScWTargetingType::Normal) ? Settings->GetGamepadLookSensitivityPreset() : Settings->GetGamepadTargetingSensitivityPreset();
...
//////////////////////////////////////////////////////////////////////
// UScWInputModifierAimInversion

FInputActionValue UScWInputModifierAimInversion::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	UScWLocalPlayer* LocalPlayer = FScWInputModifiersHelpers::GetLocalPlayer(PlayerInput);
	if (!LocalPlayer)
	{
