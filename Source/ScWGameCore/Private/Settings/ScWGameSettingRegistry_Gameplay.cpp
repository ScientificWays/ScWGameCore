...
UGameSettingCollection* UScWGameSettingRegistry::InitializeGameplaySettings(UScWLocalPlayer* InLocalPlayer)
{
...

	{
		...
		//----------------------------------------------------------------------------------
		{
			UScWSettingValueDiscrete_Language* Setting = NewObject<UScWSettingValueDiscrete_Language>();
	...
	/*{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UScWSettingsLocal>()->ShouldAutoRecordReplays());

			Setting->AddEditCondition(FWhenPlayingAsPrimaryPlayer::Get());
			Setting->AddEditCondition(FWhenPlatformHasTrait::KillIfMissing(UScWReplaySubsystem::GetPlatformSupportTraitTag(), TEXT("Platform does not support saving replays")));
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UScWSettingsLocal>()->GetNumberOfReplaysToKeep());
