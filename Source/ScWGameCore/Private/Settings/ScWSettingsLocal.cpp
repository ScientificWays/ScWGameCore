...
#include "Settings/ScWPlatformEmulationSettings.h"
...
#include "Audio/ScWAudioSettings.h"
#include "Audio/ScWAudioMixEffectsSubsystem.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(ScWSettingsLocal)
...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("ScW.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("ScW.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("ScW.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("ScW.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("ScW.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("ScW.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("ScW.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("ScW.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("ScW.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("ScW.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FScWScalabilitySnapshot::FScWScalabilitySnapshot()
{
...
namespace FScWSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FScWScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
	...
	int32 ConstrainFrameRateToBeCompatibleWithOverallQuality(int32 FrameRate, int32 OverallQuality)
	{
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = UScWSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UScWSettingsLocal::GetDefaultMobileFrameRate();
	}
...
UScWSettingsLocal::UScWSettingsLocal()
{
	...
	bEnableScalabilitySettings = UScWPlatformSpecificRenderingSettings::Get()->bSupportsGranularVideoQualitySettings;

	SetToDefaults();
}

void UScWSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
	...
	bEnableLatencyTrackingStats = UScWSettingsLocal::DoesPlatformSupportLatencyTrackingStats();
...
void UScWSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const UScWPlatformSpecificRenderingSettings* PlatformSettings = UScWPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EScWFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	FScWSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void UScWSettingsLocal::ResetToCurrentSettings()
{
...
void UScWSettingsLocal::BeginDestroy()
{
...
UScWSettingsLocal* UScWSettingsLocal::Get()
{
	return GEngine ? CastChecked<UScWSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UScWSettingsLocal::ConfirmVideoMode()
{
...
float UScWSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UScWPlatformSpecificRenderingSettings* PlatformSettings = UScWPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == EScWFramePacingMode::DesktopStyle)
	{
...
int32 UScWSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return FScWSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UScWSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FScWScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UScWSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UScWSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UScWSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UScWSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UScWSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UScWSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
EScWStatDisplayMode UScWSettingsLocal::GetPerfStatDisplayState(EScWDisplayablePerformanceStat Stat) const
{
	if (const EScWStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return EScWStatDisplayMode::Hidden;
	}
}

void UScWSettingsLocal::SetPerfStatDisplayState(EScWDisplayablePerformanceStat Stat, EScWStatDisplayMode DisplayMode)
{
	if (DisplayMode == EScWStatDisplayMode::Hidden)
	{
...
bool UScWSettingsLocal::DoesPlatformSupportLatencyMarkers()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyMarkers);
}

void UScWSettingsLocal::SetEnableLatencyFlashIndicators(const bool bNewVal)
{
...
void UScWSettingsLocal::SetEnableLatencyTrackingStats(const bool bNewVal)
{
...
void UScWSettingsLocal::ApplyLatencyTrackingStatSetting()
{
...
bool UScWSettingsLocal::DoesPlatformSupportLatencyTrackingStats()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyStats);
}

float UScWSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UScWSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UScWSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UScWSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UScWSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UScWSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UScWSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UScWSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UScWSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UScWSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UScWSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UScWSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 UScWSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UScWSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UScWSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UScWSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return FScWSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UScWSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return FScWSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UScWSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UScWSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UScWPlatformSpecificRenderingSettings* PlatformSettings = UScWPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == EScWFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return FScWSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void UScWSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UScWPlatformSpecificRenderingSettings* PlatformSettings = UScWPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EScWFramePacingMode::MobileStyle)
	{
...
void UScWSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UScWSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UScWPlatformSpecificRenderingSettings* PlatformSettings = UScWPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EScWFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = FScWSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UScWSettingsLocal::ClampMobileQuality()
{
	const UScWPlatformSpecificRenderingSettings* PlatformSettings = UScWPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EScWFramePacingMode::MobileStyle)
	{
...
void UScWSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = FScWSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UScWSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = FScWSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = FScWSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UScWSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UScWSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void UScWSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool UScWSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool UScWSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = FScWSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool UScWSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void UScWSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			if (UScWAudioMixEffectsSubsystem* ScWAudioMixEffectsSubsystem = World->GetSubsystem<UScWAudioMixEffectsSubsystem>())
			{
				ScWAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			}
		}
	}
}

bool UScWSettingsLocal::CanRunAutoBenchmark() const
{
	const UScWPlatformSpecificRenderingSettings* PlatformSettings = UScWPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UScWSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UScWSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void UScWSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float UScWSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void UScWSettingsLocal::SetOverallVolume(float InVolume)
{
...
float UScWSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UScWSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UScWSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UScWSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UScWSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void UScWSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UScWSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UScWSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UScWSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void UScWSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void UScWSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UScWSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UScWSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UScWSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == EScWFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = FScWSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UScWSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UScWSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void UScWSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const UScWAudioSettings* ScWAudioSettings = GetDefault<UScWAudioSettings>())
			{
				...
				if (UObject* ObjPath = ScWAudioSettings->OverallVolumeControlBus.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Overall Control Bus reference missing from ScW Audio Settings."));
					}
				}

				if (UObject* ObjPath = ScWAudioSettings->MusicVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Music Control Bus reference missing from ScW Audio Settings."));
					}
				}

				if (UObject* ObjPath = ScWAudioSettings->SoundFXVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("SoundFX Control Bus reference missing from ScW Audio Settings."));
					}
				}

				if (UObject* ObjPath = ScWAudioSettings->DialogueVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Dialogue Control Bus reference missing from ScW Audio Settings."));
					}
				}

				if (UObject* ObjPath = ScWAudioSettings->VoiceChatVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("VoiceChat Control Bus reference missing from ScW Audio Settings."));
					}
				}

				if (UObject* ObjPath = ScWAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from ScW Audio Settings."));
					}
				}
			}
		}
...
void UScWSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UScWSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const UScWPlatformSpecificRenderingSettings* PlatformSettings = UScWPlatformSpecificRenderingSettings::Get();
	const TArray<FScWQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FScWQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const UScWPlatformEmulationSettings* Settings = GetDefault<UScWPlatformEmulationSettings>();
	...
	{
	case EScWFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case EScWFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case EScWFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void UScWSettingsLocal::UpdateConsoleFramePacing()
{
...
void UScWSettingsLocal::UpdateDesktopFramePacing()
{
...
void UScWSettingsLocal::UpdateMobileFramePacing()
{
...
void UScWSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
