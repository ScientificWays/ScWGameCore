...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ScWGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EScWEditorLoadMode
{
...
namespace ScWGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("ScW.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via ScWGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UScWGameplayCueManager::DumpGameplayCues));

	static EScWEditorLoadMode LoadMode = EScWEditorLoadMode::LoadUpfront;
}
...
UScWGameplayCueManager* UScWGameplayCueManager::Get()
{
	return Cast<UScWGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UScWGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UScWGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogScWGameCore, Warning, TEXT("UScWGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UScWGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (ScWGameplayCueManagerCvars::LoadMode)
	{
	case EScWEditorLoadMode::LoadUpfront:
		return true;
	case EScWEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EScWEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UScWGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UScWGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UScWGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UScWGameplayCueManager* GCM = Cast<UScWGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogScWGameCore, Error, TEXT("DumpGameplayCues failed. No UScWGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogScWGameCore, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogScWGameCore, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogScWGameCore, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogScWGameCore, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogScWGameCore, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogScWGameCore, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogScWGameCore, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogScWGameCore, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogScWGameCore, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogScWGameCore, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogScWGameCore, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogScWGameCore, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UScWGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UScWGameplayCueManager* StrongThis = Get())
					{
...
void UScWGameplayCueManager::HandlePostGarbageCollect()
{
...
void UScWGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogScWGameCore, Warning, TEXT("UScWGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UScWGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (ScWGameplayCueManagerCvars::LoadMode)
	{
	case EScWEditorLoadMode::LoadUpfront:
...
void UScWGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UScWGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UScWGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UScWGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UScWGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UScWGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
