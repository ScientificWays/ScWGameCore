...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ScWWorldSettings)

AScWWorldSettings::AScWWorldSettings(const FObjectInitializer& InObjectInitializer)
...
#if WITH_EDITOR
void AScWWorldSettings::CheckForErrors() // AActor
{
	...
	for (TActorIterator<APlayerStart> PlayerStartIt(GetWorld()); PlayerStartIt; ++PlayerStartIt)
	{
		...
		/*if (IsValid(PlayerStart) && PlayerStart->GetClass() == APlayerStart::StaticClass())
		{
			MapCheck.Warning()
				...
				->AddToken(FTextToken::Create(FText::FromString("is a normal APlayerStart, replace with AScWPlayerStart.")));
		}*/
	}
