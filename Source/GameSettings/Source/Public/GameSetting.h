...
public:
	...
	MODULE_API const FString& GetDescriptionPlainText() const;
	 ...
	 */
	UFUNCTION(BlueprintCallable)
	MODULE_API FText GetDynamicDetails() const;
	...
	MODULE_API void AddEditCondition(const TSharedRef<FGameSettingEditCondition>& InEditCondition);

	/** Add setting dependency, if these settings change, we'll re-evaluate edit conditions for this setting. */
	MODULE_API void AddEditDependency(UGameSetting* DependencySetting);

	/** The parent object that owns the setting, in most cases the collection, but for top level settings the registry. */
	MODULE_API void SetSettingParent(UGameSetting* InSettingParent);
	 ...
	 */
	MODULE_API void RefreshEditableState(bool bNotifyEditConditionsChanged = true);
...
protected:
	/**  */
	MODULE_API virtual void Startup();
	MODULE_API void StartupComplete();

	MODULE_API virtual void OnInitialized();
	MODULE_API virtual void OnApply();
	MODULE_API virtual void OnGatherEditState(FGameSettingEditableState& InOutEditState) const;
	MODULE_API virtual void OnDependencyChanged();

	/**  */
	MODULE_API virtual FText GetDynamicDetailsInternal() const;

	/** */
	MODULE_API void HandleEditDependencyChanged(UGameSetting* DependencySetting, EGameSettingChangeReason Reason);
	MODULE_API void HandleEditDependencyChanged(UGameSetting* DependencySetting);

	/** Regenerates the plain searchable text if it has been dirtied. */
	MODULE_API void RefreshPlainText() const;
	void InvalidateSearchableText() { bRefreshPlainSearchableText = true; }

	/** Notify that the setting changed */
	MODULE_API void NotifySettingChanged(EGameSettingChangeReason Reason);
	MODULE_API virtual void OnSettingChanged(EGameSettingChangeReason Reason);
	...
	MODULE_API void NotifyEditConditionsChanged();
	MODULE_API virtual void OnEditConditionsChanged();

	/**  */
	MODULE_API FGameSettingEditableState ComputeEditableState() const;
